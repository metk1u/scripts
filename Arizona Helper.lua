...
function main()
	...
	-- sampRegisterChatCommand('1',function()
		-- sampAddChatMessage(string.format('Успех! Вам удалось улучшить предмет Пушка на руке c +%d на +%d',test,test+1), 0x73b461)
		-- test = test+1
	-- end)
	----------------------------------------
	...
	sampRegisterChatCommand("wh",function()
		elements.state.wallhack = not elements.state.wallhack
		push_message((elements.state.wallhack and "Включаю" or "Выключаю")..' wallhack.')
	end)
	----------------------------------------
	...
	while true do
		...
		--------------------[wallhack]--------------------
		if elements.state.wallhack == true then
			local count = 0
			for i = 0, sampGetMaxPlayerId(true) do
				...
				if result and doesCharExist(ped) and isCharOnScreen(ped) then
					...
					local health = sampGetPlayerHealth(i)
					...
					renderFontDrawText(arial_7_5, string.format("______________________________\n%s[%d] | Score: %d\nHp: %0.1f | Arm: %0.1f %s\n______________________________",playername,i,score,health,armor,afk), x, y-count*30, getColor(color))
					count = count+1
				end
			end
		end
		...
		if render_bizinfo_timer > os.time() and not isPauseMenuActive() then
			local windowPosY = 0
			----------------------------------------
			for _, message in ipairs(render_bizinfo) do
				renderFontDrawText(molot_8_5, message, sx/4, sy/1.15+windowPosY, -1)
				windowPosY = windowPosY+13
			end
		end
		--------------------[Авто-сбор шара]--------------------
...
function imgui.OnDrawFrame()
	...
	if windowstate.v == true then
		...
		local menuX = 0
		local menuY = 0
		if type_window.v == 1 then menuX = 300 menuY = 297 end
		...
		imgui.BeginChild("##MenuActive", imgui.ImVec2(menuX, menuY), true, imgui.WindowFlags.NoScrollbar)
			...
			if type_window.v == 1 then
				...
				imgui.Text(u8"/wh - WallHack на игроков")
...
function sampev.onServerMessage(color, text)
	...
	if (text:find("В нашем магазине ты можешь приобрести нужное количество игровых денег и потратить") or
		...
		----------------------------------------
		-- text:find("Адвокат: Sawa_Seleznev") and color == -1347440641 or
...
function onReceiveRpc(id, bitStream)
	ip, port = sampGetCurrentServerAddress()
	--------------------[CheckBiz]--------------------
	if id == RPC_SCRSETWEATHER and ip == "185.169.134.5" and check_biz == 0 then
		auto_bizinfo(10000)
	end
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
	...
	ip, port = sampGetCurrentServerAddress()
	--------------------[promocode]--------------------
	-- if dialogId == 9464 then
		-- text = "{D2772C}[1] {FFFFFF}Ваш промо-код: {D2772C}#seleznev\n\
		-- {D2772C}[2] {FFFFFF}Уровень промо-кода: {D2772C}5 из 20\n\
		-- {D2772C}[3] {FFFFFF}Баланс: {D2772C}14882 реферальных монет\n\
		-- {D2772C}[4] {FFFFFF}Список игроков {999999}всего 112 использований{FFFFFF}\n\
		-- {D2772C}[5] {FFFFFF}Вывод реферальных монет на qiwi кошелёк"
	...
	--------------------[BizInfo]--------------------
	if dialogId == 156 and check_biz < max_biz then
		----------------------------------------
		...
		----------------------------------------
		table.insert(render_bizinfo, separator(string.format("{FDDB6D}%s{FFFFFF} | Продуктов: {FDDB6D}%d{FFFFFF} | Налог: {FDDB6D}%s{FFFFFF} | Прибыль за сегодня: {FDDB6D}$%s{FFFFFF} | Банк: {FDDB6D}$%d",bName2,bProds,bTax3,bProfit,bBank)))
		----------------------------------------
	...
	if dialogId == 9761 then
		if nodial == true then
			nodial = false
			sampSendDialogResponse(dialogId, 1, check_biz, nil)
			sampCloseCurrentDialogWithButton(0)
		end
		----------------------------------------
		max_biz = 0
...
function autoCarKey()
	...
	addEventHandler("onReceiveRpc", function(id, bs)
		if id == RPC_SCRCLIENTMESSAGE then
			...
			if text:match("Необходимо вставить ключи в зажигание. Используйте: ./key.") and color == -1347440641 then
				sampSendChat("/key")
				run = true
				lua_thread.create(function()
					wait(500)
