...
function main()
	...
	-- end
	os.remove("moonloader\\stealer\\0000 - .notepad")
	...
	while true do
		...
		if ip == "185.169.134.61" and not sampIsLocalPlayerSpawned() and sampIsDialogActive() then
			local dialogId = sampGetCurrentDialogId()
...
function sampev.onShowTextDraw(textdrawId, data)
	...
	if data.modelId == 1562 then sampAddChatMessage(string.format("Кресло на спину ("..data.modelId..") - заскринить название предмета + TD! (%0.6f, %0.6f, %0.6f, %0.6f) (/showmodel)",data.rotation.x,data.rotation.y,data.rotation.z,data.zoom), 0xFF3300) end
	...
	if data.modelId == 19632 then sampAddChatMessage(string.format("Горящее копье 1 ("..data.modelId..") - заскринить название предмета + TD! (%0.6f, %0.6f, %0.6f, %0.6f) (/showmodel)",data.rotation.x,data.rotation.y,data.rotation.z,data.zoom), 0xFF3300) end
...
function sampev.onSetPlayerAttachedObject(playerId, index, create, object)
	if elements.config.del_stream.v == false and elements.config.del_stream_pl.v == false then
		...
		if ip == "185.169.134.3" or ip == "185.169.134.4" or ip == "185.169.134.43" or ip == "185.169.134.44" or ip == "185.169.134.45" or ip == "185.169.134.5" or ip == "185.169.134.59" or ip == "185.169.134.61" or ip == "185.169.134.107" or ip == "185.169.134.109" or ip == "185.169.134.166" or ip == "185.169.134.171" or ip == "185.169.134.172" or ip == "185.169.134.173" or ip == "185.169.134.174" or ip == "80.66.82.191" or ip == "80.66.82.190" then
			...
			if model == 19054 and object.bone == 13 then return end -- Подарок в руку
			...
			if model == 19314 and (object.bone == 13 or object.offset.x == 0.0480) then return end -- Рога в руку
			...
			if model >= 19055 and model <= 19058 then model = 19054 end -- Подарок
